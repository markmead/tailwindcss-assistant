(()=>{function L(){let e=document.createElement("script");e.setAttribute("src","https://cdn.tailwindcss.com?plugins=forms,typography,aspect-ratio,line-clamp"),document.head.appendChild(e)}var l="twaAddClassForm",d="twaAddClassInput";function M(e){e.classList.add(document.getElementById(d).value),W()}function w(e){document.getElementById(l).addEventListener("submit",function(t){t.preventDefault(),M(e)})}function W(){document.getElementById(d).value=""}var c="twaClassInputsRender";function x(){return[...document.querySelectorAll("[data-class]")]}function I(e){U(),e.forEach(function(t){document.getElementById(c).innerHTML+=H(t)})}function C(e){x().forEach(t=>{t.addEventListener("input",()=>{e.classList.toggle(t.name),document.querySelector(`[for='${t.id}']`).classList.toggle("opacity-25")})})}function U(){document.getElementById(c).innerHTML=""}function H(e){return`
    ...
    `;let s,o=document.getElementById("twaBreakpoint"),r,B=[];document.addEventListener("click",i=>{twaPopup.contains(i.target)||(twaPopup.open=!1),i.metaKey&&(i.preventDefault(),twaPopup.open=!0,s=i.target,o.innerText=E(),r=h(s),N(),g(),b(),O(),D())});function N(){w(s),document.getElementById(l).addEventListener("submit",g)}function g(){B=[...s.classList],I(B),twaClassInputs=x(),C(s)}function b(){y(),twaBreakpointInputs=v(),k(s,r)}function O(){S(),T(t,e)}function D(){$(),R(s)}document.addEventListener("twa:updated-target",function(i){s=i.detail.newTarget,r=h(s),g(),b()}),window.addEventListener("resize",function(){o.innerText=E()})}document.addEventListener("DOMContentLoaded",()=>{_()});})();
