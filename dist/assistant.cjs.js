var y=Object.defineProperty;var _=n=>y(n,"__esModule",{value:!0});var W=(n,l)=>{_(n);for(var a in l)y(n,a,{get:l[a],enumerable:!0})};W(exports,{default:()=>N});var m="twaAddClassForm",f="twaAddClassInput";function R(n){n.classList.add(document.getElementById(f).value),M()}function S(n){document.getElementById(m).addEventListener("submit",function(l){l.preventDefault(),R(n)})}function M(){document.getElementById(f).value=""}var g="twaClassInputsRender";function C(){return[...document.querySelectorAll("[data-class]")]}function k(n){q(),n.forEach(function(l){document.getElementById(g).innerHTML+=F(l)})}function A(n,l){C().forEach(a=>{a.addEventListener("input",()=>{n.classList.toggle(a.name),document.querySelector(`[for='${a.id}']`).classList.toggle("opacity-25")})})}function q(){document.getElementById(g).innerHTML=""}function F(n){return`
    ...
    `,a.classList.add("fixed");let p=["right-4","bottom-4"];p.forEach(e=>a.classList.add(e)),document.body.appendChild(a);let s,E=document.getElementById("twaBreakpoint"),x=[...document.querySelectorAll("[data-breakpoint]")],B=document.getElementById("twaClassesEditor"),P=[...document.querySelectorAll("[data-position]")],T=[...document.querySelectorAll("[data-relative]")],h=document.getElementById("twaError"),v,L=[];document.addEventListener("click",e=>{twaPopup.contains(e.target)||(twaPopup.open=!1),e.metaKey&&(e.preventDefault(),twaPopup.open=!0,s=e.target,E.innerText=n(),x.forEach(t=>t.checked=!0),D(),w(),v=$(s))});function D(){S(s),document.getElementById(m).addEventListener("submit",w)}function w(){L=[...s.classList],k(L),twaClassInputs=C(),A(s,B)}x.forEach(e=>{e.addEventListener("input",()=>{v[e.name].forEach(function(t){s.classList.toggle(t),document.querySelector(`[for='${t}']`).classList.toggle("opacity-25")})})}),T.forEach(e=>{e.addEventListener("click",()=>{let t,r=e.getAttribute("data-relative");r==="parent"&&(t=s.parentElement?s.parentElement:s,!s.parentElement&&u("No parent element")),r==="prev"&&(t=s.previousElementSibling?s.previousElementSibling:s,!s.previousElementSibling&&u("No previous sibling element")),r==="next"&&(t=s.nextElementSibling?s.nextElementSibling:s,!s.nextElementSibling&&u("No next sibling element")),r==="child"&&(t=s.firstElementChild?s.firstElementChild:s,!s.firstElementChild&&u("No child element")),s=t,x.forEach(b=>b.checked=!0),v=$(s),w()})}),window.addEventListener("resize",()=>{E.innerText=n()}),P.forEach(e=>{e.addEventListener("click",()=>{p.forEach(t=>a.classList.remove(t)),p=e.getAttribute("data-position").split(","),p.forEach(t=>a.classList.add(t))})});let u=e=>{h.removeAttribute("hidden"),h.innerText=e,setTimeout(()=>{h.setAttribute("hidden",!0)},3e3)},$=e=>({"2xl":[...e.classList].filter(t=>t.startsWith("2xl:")),xl:[...e.classList].filter(t=>t.startsWith("xl:")),lg:[...e.classList].filter(t=>t.startsWith("lg:")),md:[...e.classList].filter(t=>t.startsWith("md:")),sm:[...e.classList].filter(t=>t.startsWith("sm:")),dark:[...e.classList].filter(t=>t.startsWith("dark:"))})}var N=I;0&&(module.exports={});
